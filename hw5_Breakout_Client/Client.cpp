...
int main(int argc, char* argv[])
{
	...
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(70.f, 50.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(240.f, 50.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(410.f, 50.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(580.f, 50.f)));

	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(70.f, 105.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(240.f, 105.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(410.f, 105.f)));
	blocks.push_back(Platform(sf::Vector2f(150.f, 50.f), sf::Vector2f(580.f, 105.f)));
	...
	platform.push_back(Platform(sf::Vector2f(800.f, 100.f), sf::Vector2f(0.f, 580.f)));
	...
	Character character(atoi(argv[1]), sf::Vector2f(20.f, 20.f), sf::Vector2f(100.f, 400.f), true, sf::Color::Yellow, 60);
	Character clients(10, sf::Vector2f(100.f, 20.f), sf::Vector2f(50.f, 550.f), true, sf::Color::Yellow, 1);
	...
	vector<sf::FloatRect> boundingBoxPlatform(platform.size());
	vector<sf::FloatRect> boundingBoxBlocks(blocks.size());
	sf::FloatRect boundingBoxClient;
	...
	while (window.isOpen())
	{
		...
		if (blocks.size() == 0) {
			...
			text.setString("CONGRATS YOU WON!!!!");
			...
			text.setPosition(sf::Vector2f(350.f, 300.f));
		...
		if (!flagPause) {
			...
			for (auto i = 0; i < blocks.size(); i++) {
				boundingBoxBlocks[i] = blocks[i].getGlobalBounds();
			}
			boundingBoxClient = clients.getGlobalBounds();
			...
			if (!flagInfocus) {
				...
				if (sf::Keyboard::isKeyPressed(sf::Keyboard::Left))
				{
					...
					clients.move(sf::Vector2f(-15.f,0.f));
				...
				if (boundingBoxCharacter.intersects(boundingBoxPlatform[1])) {
					e.push_back(Event(C_DEATH, character.getId(), lastTime, sf::Vector2f(0.f, timeElapsed * -2.f), character.getScreenno()));
					flagClose = 1;
				}
				if (boundingBoxClient.intersects(boundingBoxPlatform[2])) {
					clients.move(sf::Vector2f(15.f, 0.f));
				}
				else if (boundingBoxClient.intersects(boundingBoxPlatform[3])) {
					clients.move(sf::Vector2f(-15.f, 0.f));
				}

				for (int i = 0; i < blocks.size(); i++) {
					if (boundingBoxCharacter.intersects(boundingBoxBlocks[i])) {
						blocks.erase(blocks.begin() + i);
						if(character.getScreenno() < 180)
							character.setScreenno((360 - character.getScreenno()) % 360);
					}
				}

				if (boundingBoxCharacter.intersects(boundingBoxPlatform[2]) || boundingBoxCharacter.intersects(boundingBoxPlatform[3])) {
						character.setScreenno(( 180 - character.getScreenno()) % 360);
						cout << "2,3 :" << character.getScreenno() << endl;
				}
			
				if (boundingBoxCharacter.intersects(boundingBoxPlatform[0]) || boundingBoxCharacter.intersects(boundingBoxClient)) {
					character.setScreenno((360 - character.getScreenno()) % 360);
					cout << "1 :" << character.getScreenno() << endl;
				}

				character.move(sf::Vector2f(10.0 * cos(character.getScreenno() * PI / 180.0), -10.0 * sin(character.getScreenno() * PI / 180.0)));
			}
