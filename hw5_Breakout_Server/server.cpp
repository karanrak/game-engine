...
sf::Vector2f foodPos = sf::Vector2f(400.f,400.f);
...
//handle it
			...
			if (e.getType() == C_COLLIDE) {
				float numx = (((MainTime.getTime() % 899) / 30) + 5) * 20.0;
				float numy = (((MainTime.getTime() % 323) / 20) + 5) * 20.0;
				foodPos.x = numx;
				foodPos.y = numy;
				cout << "X " << numx <<" Y " << numy;
			}
...
//Handle the event
			...
			lastPos = clientMap[client_id];
			tempStr = to_string(foodPos.x) + " " + to_string(foodPos.y) + " " + to_string(charMap[client_id].getScreenno());
